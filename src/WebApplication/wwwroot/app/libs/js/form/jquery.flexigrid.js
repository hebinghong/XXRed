...
(function($) {
    $.addFlex = function(t, p) {
        ...
        p = $.extend({
            ...
            rp: 23, // results per page,每页默认的结果数
                    ...
                    if (data.rows != null) {
                        $.each(data.rows, function(i, row) {
                            ...
                            $(ths).each(function(j) {
                                ...
                                var titleabbr=$(this).attr("titleabbr");
                                ...
                                if (idx != "-1") {
                                    ...
                                    titledata=tddata;
                                     ...
                                     if (titleabbr.indexOf(".") > -1) {
                                        var arrabbr = titleabbr.split(".");
                                        var arr = [];
                                        for (var k = 0, len = arrabbr.length; k < len; k++) {
                                            var subKey = arrabbr[k];
                                            if (arr.length) {
                                ...
                                if (idx == "-1") { //checkbox
                                    ...
                                    if (this.process) {
                                        divInner = this.process(divInner, key);
                                    }
                                    if (linkurl) {
                        ...
                        if (this.name == "curparms") {
                            if ($.isArray(this.value)) {
                                var newformVals = $.grep(this.value, function(n, i) {
                                    if (n.value && (n.name != "rp" || n.name != "page" || n.name != "sortname"
                        ...
                        if (data) {
                            if (data != null && data.error != null && data == "undefined") {
                                ...
                                if (typeof Dialog == "function")
                                    Dialog.alert(data.errMsgs.join(""));
                                else
        ...
        if (p.colModel) {
            ...
            for (i = 0; i < p.colModel.length; i++) {
                ...
                if (cm.name)
                    ...
                    if(cm.titlename)
                    $(th).attr('titleabbr', cm.titlename);
                    else
                    $(th).attr('titleabbr', cm.name);
                if (cm.sortable)
                ...
                if (!btn.hide) {
                    if (!btn.separator && btn.displayname) {
                        ...
                        if (btn.onpress) {
							...
							(
								function() {
								    var selectTr = $(".trSelected", g.gDiv), ids = [];
								    selectTr.each(function() {
								        var cur = $(this);
								        ids.push(cur.attr("id").substr(3));
								    });
								    this.onpress(this.name, selectTr, ids);
								}
							);
                        }
                        $(tDiv2).append(btnDiv);
